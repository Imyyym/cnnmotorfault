...
class Individual:
    ...
    def evaluate_fitness(self):
        ...
        for i in range(gene_len - 1):
            ...
            a=0
            b=0
            if from_idx == 14 and to_idx == 13:
                a=1
            if from_idx == 13 and to_idx == 14:
                a=1
            if from_idx == 7 and to_idx == 13:
                b=100
            if from_idx == 13 and to_idx == 7:
                b = 100
            fitness += city_dist_mat[from_idx, to_idx]+a+b
        # 连接首尾
...
class Ga:
    ...
    def cross(self):
        ...
        for i in range(0, individual_num - 1, 2):
            # 父代基因
            genes1 = copy_list(self.individual_list[i].genes)#复制第i个基因，是一个list
            genes2 = copy_list(self.individual_list[i + 1].genes)#复制第i+1个基因
            index1 = random.randint(0, gene_len - 2)#零到城市的个数的随机值整数
            index2 = random.randint(index1, gene_len - 1)
            pos1_recorder = {value: idx for idx, value in enumerate(genes1)}#在genes1中的序号和值
            pos2_recorder = {value: idx for idx, value in enumerate(genes2)}#在genes2中的序号和值
            # 交叉
            for j in range(index1, index2):#交叉的段
                value1, value2 = genes1[j], genes2[j]#防止重复，要不然就直接换了
                pos1, pos2 = pos1_recorder[value2], pos2_recorder[value1]
                genes1[j], genes1[pos1] = genes1[pos1], genes1[j]#先在一个基因里交换
                genes2[j], genes2[pos2] = genes2[pos2], genes2[j]
                pos1_recorder[value1], pos1_recorder[value2] = pos1, j#数值换了序号也要换
